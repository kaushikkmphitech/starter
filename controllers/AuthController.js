...
export const login = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ key: req.key, userId: user._id }, config.auth.jwtSecret, { expiresIn: config.auth.jwtExpiresIn });
        ...
        return await successResponse(res, 200, 1, config.messages.loginSuccessful, response);
    } catch (error) {
...
export const register = async (req, res) => {
    ...
    try {
        ...
        const str = await ejs.renderFile('views/emails/confirmation.ejs', mailData, { async: true });
        const mailOptions = {
        ...
        return await successResponse(res, 201, 1, config.messages.registrationSuccessful);
    } catch (error) {
        await session.abortTransaction();
        return await errorResponse(res, 500, 0, error.message);
    } finally {
...
export const verifyEmail = async (req, res) => {
    ...
    try {
        ...
        if(!user) {
            return await errorResponse(res, 400, 0, config.messages.invalidTokenOnVerifyEmail);
        }
        ...
        return await successResponse(res, 200, 1, config.messages.emailAlreadyVerified);
    } catch (error) {
        await session.abortTransaction();
        return await errorResponse(res, 500, 0, error.message);
    } finally {
...
export const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        const str = await ejs.renderFile('views/emails/forgot-password.ejs', mailData, { async: true });
        const mailOptions = {
            from: "Starter <starter@starter.com>",
            to: req.body.email,
        ...
        return await successResponse(res, 200, 1, config.messages.forgotPasswordSuccessful);
    } catch (error) {
        await session.abortTransaction();
        return await errorResponse(res, 500, 0, error.message);
    } finally {
...
export const resetPassword = async (req, res) => {
    ...
    try {
        ...
        const user = await User.findOne({ token });
        if(!user) {
            return await errorResponse(res, 400, 0, config.messages.invalidTokenOnResetPassword);
        }
        ...
        return await successResponse(res, 200, 1, config.messages.resetPasswordSuccessful);
    } catch (error) {
        await session.abortTransaction();
        return await errorResponse(res, 500, 0, error.message);
    } finally {
